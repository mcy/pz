...
impl GenFieldImpl for Singular {
  ...
  fn in_variants(&self, field: Field, w: &mut SourceWriter) {
    w.emit(
      ...
      "
        $Name(__rt::ptr::Proxy<'proto, $Scalar, Which>),
      ",
    );
  }
  ...
  fn in_ref_methods(&self, field: Field, at: Where, w: &mut SourceWriter) {
    w.emit(
      ...
      r"
        $deprecated
        pub fn $name($self) -> $View<'$lt, $Scalar> {
          self.${name}_or().unwrap_or_default()
        }
        $deprecated
        pub fn ${name}_or($self) -> $Option<$View<'$lt, $Scalar>> {
          unsafe {
  ...
  fn in_mut_methods(&self, field: Field, at: Where, w: &mut SourceWriter) {
    w.emit(
      ...
      r"
        $deprecated
        pub fn ${name}_mut($self) -> $Mut<'$lt, $Scalar> {
          self.${name}_mut_or().into_mut()
        }
        $deprecated
        pub fn ${name}_mut_or($self) -> __rt::OptMut<'$lt, $Scalar> {
          unsafe {
            __rt::OptMut::__wrap(
              self.ptr.as_ptr(),
              self.arena,
...
impl GenFieldImpl for Repeated {
  fn in_storage(&self, field: Field, w: &mut SourceWriter) {
    w.emit(
      vars! { Storage: scalar_storage_type(field) },
      "
        pub(in super) $name: __z::AVec<$Storage>,
      ",
    );
  }
  ...
  fn in_variants(&self, field: Field, w: &mut SourceWriter) {
    w.emit(
      vars! { Scalar: scalar_type(field) },
      "
        $Name(__rt::ptr::Proxy<'proto, __rt::ptr::Rep<$Scalar>, Which>),
      ",
    );
  }
  ...
  fn in_ref_methods(&self, field: Field, at: Where, w: &mut SourceWriter) {
    w.emit(
      ...
      r"
        $deprecated
        pub fn $name($self) -> $Slice<'$lt, $Scalar> {
          unsafe {
            let field = $TDP.field($idx);
        ...
        $deprecated
        pub fn ${name}_at($self, idx: usize) -> $View<'$lt, $Scalar> {
          self.$name().at(idx)
        }
      ",
    );
  ...
  fn in_mut_methods(&self, field: Field, at: Where, w: &mut SourceWriter) {
    w.emit(
      ...
      r"
        $deprecated
        pub fn ${name}_mut($self) -> $Repeated<'$lt, $Scalar> {
          unsafe {
            let field = $TDP.field($idx);
